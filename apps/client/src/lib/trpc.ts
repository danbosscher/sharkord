...
type TCleanupOptions = {
  preserveAutoLoginToken?: boolean;
  preserveSessionToken?: boolean;
};
...
const isForcedDisconnectCode = (code?: number) => {
...
const initializeTRPC = (host: string) => {
  ...
  wsClient = createWSClient({
    ...
    onClose: (cause: CloseEvent) => {
      ...
      if (isForcedDisconnectCode(cause?.code)) {
        cleanup();
...
const cleanup = (options?: TCleanupOptions) => {
  if (isCleaningUp) {
    return;
  }
  ...
  if (!isNavigatingAway && !options?.preserveAutoLoginToken)
